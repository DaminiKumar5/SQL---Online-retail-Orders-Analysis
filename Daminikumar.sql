...
Select * from product order by product_class_code desc;
select product_class_code,product_price,product_id,product_desc, case 
...
select pc.product_class_desc,p.product_id, p.product_quantity_avail,p.product_desc,p.product_class_code, 
...
/* 3. Write a query to show the number of cities in all countries other than USA & MALAYSIA, with more than 1 city, in the descending order of CITIES.*/
...
/* 4. Write a query to display the customer_id,customer full name ,city,pincode,and order details (order id, product class desc, product desc, subtotal(product_quantity * product_price)) for orders shipped to cities whose pin codes do not have any 0s in them. Sort the output on customer name and subtotal. */

#-----------SOLUTION 4-----------------------

select * from ORDER_HEADER;
select o.customer_id,od.customer_id,o.customer_fname,a.pincode,od.order_id
,pc.product_class_desc,p.product_desc,oi.product_quantity * p.product_price subtotal
from
    address a 
inner join
    online_customer o
...
where
    a.pincode not like '%0%' and od.order_status = 'Shipped'
order by
    o.customer_fname,subtotal;
    
   
  /* 5. Write a Query to display product id,product description,totalquantity(sum(product quantity) for a given item whose product id is 201 and which item has been bought along with it maximum no. of times. Display only one record which has the maximum value for total quantity in this scenario.*/
...
Select p.product_id, p.product_desc, sum(o.product_quantity), max(o.product_quantity) from product p
join order_items o
on o.product_id =p.product_id 
where p.product_id = '201';


/* 6. Write a query to display the customer_id,customer name, email and order details (order id, product desc,product qty, subtotal(product_quantity * product_price)) for all customers even if they have not ordered any item. */

#---------------SOLUTION 6---------------------

Select o.customer_id, concat(o.customer_fname," ",o.customer_lname) as customer_name, 
o.customer_email, od.order_id, p.product_desc, oi.product_quantity, (oi.product_quantity*p.product_price) as subtotal 
from online_customer o left join order_header od on o.customer_id = od.customer_id left join 
order_items oi on od.order_id = oi.order_id left join product p on oi.product_id = p.product_id;
...
select carton_id,(len*width*height) as carton_vol from carton having carton_vol > ((select sum((o.product_quantity*p.len*p.width*p.height)) as total_vol from order_items as o join product as p
on o.product_id=p.product_id where order_id=10006))
Order by carton_vol
Limit 1;


/* 8.Write a query to display details (customer id,customer fullname,order id,product quantity) of customers who bought more than ten (i.e. total order qty) products with credit card or Net banking as the mode of payment per shipped order. */

#-----------------SOLUTION 8----------------------

Select o.customer_id, concat(o.customer_fname," ",o.customer_lname) as customer_fullname, od.order_id, 
sum(oi.product_quantity) as total_order_quantity, od.payment_mode from online_customer o inner join 
order_header od on o.customer_id = od.customer_id inner join order_items oi on od.order_id = oi.order_id 
where od.order_status = 'shipped' and (od.payment_mode = 'net banking' or od.payment_mode = 'credit card') 
group by od.order_id having total_order_quantity>10;


/* 9. Write a query to display the order_id, customer id and cutomer full name of customers starting with the alphabet "A" along with (product_quantity) as total quantity of products shipped for order ids > 10030. */

#-----------------------SOLUTION 9-------------------

Select od.order_id, o.customer_id, concat(o.customer_fname," ",o.customer_lname) as customer_fullname,
sum(oi.product_quantity) as total_order_quantity from online_customer o inner join 
order_header od on od.customer_id = o.customer_id inner join 
order_items oi on od.order_id = oi.order_id where od.order_status = 'shipped' and od.order_id>10030 
group by order_id having customer_fullname like 'A%' order by customer_fullname;
...
Select pd.product_class_desc, sum(oi.product_quantity) as total_order_quantity, sum(oi.product_quantity*p.product_price) as total_value 
from product_class pd inner join 
product p on pd.product_class_code = p.product_class_code inner join 
order_items oi on oi.product_id = p.product_id inner join 
order_header od on od.order_id = oi.order_id inner join 
online_customer o on o.customer_id = od.customer_id inner join 
address a on a.address_id = o.address_id where od.order_status = 'shipped' and a.country not in ('India','USA') 
group by pd.product_class_desc, pd.product_class_code order by total_order_quantity desc 
limit 1;
